...
REQUIRED=["AIRI_AGENT_ENTRYPOINT.md","config/AIRI_SYSTEM_MANIFEST.json","config/AIRI_REASONING_POLICY.json","config/DEFAULT_REASONING_DIRECTIVE.md","config/AGENT_PROMPT_CONFIG.json","config/AGENT_PROMPT_AUTOLOAD.md","computer/control_plane/github_access.py","scripts/airi-rebuild-verify.py","computer/control_plane/generalist_provider.py","computer/control_plane/local_agent.py","computer/control_plane/model_router.py","computer/control_plane/model_gateway.py","scripts/airi-local-autonomous","AIRI_LOCAL_AUTONOMOUS.md","api/index.py","computer/server.py","computer/start.sh","tests/test_openrouter_provider.py"]
...
def main():
    ...
    try:
        manifest=json.loads((ROOT/"config/AIRI_SYSTEM_MANIFEST.json").read_text()); policy=json.loads((ROOT/"config/AIRI_REASONING_POLICY.json").read_text()); prompt=json.loads((ROOT/"config/AGENT_PROMPT_CONFIG.json").read_text())
    except Exception as exc:
        print(f"[ERROR] configuration JSON parse: {exc}"); failures.append("config"); manifest=policy=prompt={}
    manifest_local=manifest.get("qualified_local_reasoning") or {}
    policy_local=policy.get("local_provider") or {}
    policy_remote=policy.get("remote_providers") or {}
    checks=[
        (
            "manifest_reasoning_policy",
            manifest.get("architecture")=="chatgpt-default-qualified-local-optional"
            and manifest.get("default_reasoning_authority")=="chatgpt"
            and manifest_local.get("allowed") is True
            and manifest_local.get("provider")=="airi-generalist"
            and manifest.get("remote_reasoning_provider_allowed") is False,
        ),
        (
            "config_reasoning_policy",
            policy.get("default_provider")=="chatgpt"
            and policy.get("execution_provider")=="airi-pc"
            and policy_local.get("name")=="airi-generalist"
            and policy_local.get("allowed") is True
            and policy_local.get("enabled_by_default") is False
            and policy_local.get("qualification_required") is True
            and policy_local.get("autonomous_downloads") is False
            and policy_local.get("trust_remote_code") is False
            and policy_remote.get("arbitrary") is False
            and policy_remote.get("openrouter") is False
            and policy_remote.get("ollama") is False,
        ),
        (
            "prompt_default_chatgpt",
            prompt.get("reasoning_provider")=="chatgpt"
            and prompt.get("execution_provider")=="airi-pc",
        ),
    ]
    ...
    for p in ROOT.rglob("*"):
        ...
        if rel=="scripts/airi-rebuild-verify.py" or rel in ALLOW_LEGACY or "/tests/" in f"/{rel}" or rel.startswith("config/") or rel.startswith(".ai/"): continue
