...
# Android 14/15 may first ask whether to share one app or the entire screen.
if coords_for '(Entire screen|entire screen|Intero schermo|schermo intero)' >/tmp/airi-share-choice 2>/dev/null; then
  adb shell input tap $(cat /tmp/airi-share-choice)
  sleep 0.5
fi

tap_matching '^(Start now|Share|Share screen|Avvia ora|Condividi|Condividi schermo)$' 24
