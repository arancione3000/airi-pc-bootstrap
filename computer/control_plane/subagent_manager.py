...
class SubagentManager:
    ...
    def _repo_path(self,path):
        q=Path(path); q=(ROOT/q).resolve() if not q.is_absolute() else q.resolve()
        ...
        return q
    def _git(self,args,cwd): return subprocess.run(['git',*args],cwd=str(cwd),text=True,capture_output=True,timeout=60,check=False)
    def create(self,goal,repo_path='.',branch_prefix='agent'):
        repo=self._repo_path(repo_path); branch=f"{branch_prefix}/{uuid.uuid4().hex[:8]}"; worktrees=repo/'.ai'/'worktrees'; worktrees.mkdir(parents=True,exist_ok=True); wt=worktrees/branch.replace('/','-'); p=self._git(['worktree','add','-b',branch,str(wt),'HEAD'],repo)
        if p.returncode!=0: raise RuntimeError(p.stderr.strip() or 'git worktree creation failed')
        aid=uuid.uuid4().hex[:12]; row={'id':aid,'goal':goal,'branch':branch,'worktree':str(wt),'repo':str(repo),'status':'ready','created_at':now(),'updated_at':now()}; self.state['agents'][aid]=row; save_json(FILE,self.state); return row
