...
class ProjectIndex:
    ...
    def refresh(self, paths=None):
        ...
        for p in targets:
            ...
            try:
                stat=p.stat(); prev=self.state['files'].get(rel)
                if prev and prev.get('mtime_ns')==stat.st_mtime_ns and prev.get('bytes')==stat.st_size: continue
                data=p.read_text(errors='replace')
            except Exception: continue
            sha=hashlib.sha256(data.encode()).hexdigest()
            if prev and prev.get('sha256')==sha:
                prev.update({'mtime_ns': stat.st_mtime_ns, 'bytes': stat.st_size, 'updated_at': now()}); continue
            ...
            self.state['symbols'][rel]=syms; self.state.setdefault('dependencies', {})[rel]=sorted({x for x in imports if x})
    ...
    def context_pack(self, query: str, limit_files: int = 12, max_bytes: int = 120000):
        ...
        for path, meta in self.state.get('files',{}).items():
            symbols=self.state.get('symbols',{}).get(path,[]); hay=(path+' '+' '.join(symbols)).casefold()
        ...
        for rel in selected:
            p=ROOT/rel
            try:
                raw=p.read_bytes()[:max(0,max_bytes-total)]; text=raw.decode('utf-8',errors='replace')
                while len(text.encode()) > max(0,max_bytes-total): text=text[:-1]
            except Exception: continue
