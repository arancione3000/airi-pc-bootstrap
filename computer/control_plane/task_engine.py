...
class TaskEngine:
    ...
    def update(self,node_id,status,output=None,error=None,checkpoint=None):
        if status not in STATES: raise ValueError(status)
        row=self.read(); node=next(n for n in row["nodes"] if n["id"]==node_id); node.update({"status":status,"output":output,"error":error,"checkpoint":checkpoint})
        ...
        row["status"] = "failed" if any(n["status"]=="failed" for n in row["nodes"]) else row.get("status","running")
        row["updated_at"]=now(); save_json("tasks.json",self.state); return row
    def finish(self,status="completed"):
        row=self.read(); row["status"]=status; row["updated_at"]=now();
        if status=="completed":
            for n in row["nodes"]: n["status"]="completed"
