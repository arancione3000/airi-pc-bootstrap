...
class TaskEngine:
    ...
    def start(self, goal:str, nodes:list[dict]|list[str], scope:list[str]|None=None):
        ...
        for i,n in enumerate(nodes):
            ...
            n.update({"status":n.get("status","pending"),"input":n.get("input"),"output":n.get("output"),"error":None,"retry_count":n.get("retry_count",0),"checkpoint":n.get("checkpoint")}); norm.append(n)
        ids=[n["id"] for n in norm]
        if len(ids) != len(set(ids)): raise ValueError("duplicate task node id")
        known=set(ids)
        for n in norm:
            missing=[d for d in n["depends_on"] if d not in known]
            if missing: raise ValueError(f"unknown task dependency: {missing}")
        visiting, visited=set(), set(); graph={n["id"]: list(n["depends_on"]) for n in norm}
        def visit(node):
        ...
        if runnable: runnable[0]["status"]="running"
    ...
    def add_node(self, task_id, node, created_by='dynamic'):
        ...
        n=dict(node); n.setdefault('id', uuid.uuid4().hex[:10]); n.setdefault('title', n['id']); n.setdefault('depends_on',[]); n.setdefault('operation','analyze'); n.setdefault('args',{}); n.setdefault('verification',{}); n.setdefault('created_by',created_by)
    ...
    def finish(self,status="completed"):
        ...
        if status not in STATES: raise ValueError(status)
        row["status"]=status; row["updated_at"]=now(); self.state["active"]=None; save_json("tasks.json",self.state); return row
