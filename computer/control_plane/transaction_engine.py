...
class TransactionEngine:
    ...
    def step(self,tid,label,note='',tool=None,input_data=None,result=None,error=None):
        row=self.state['transactions'][tid]; row['steps'].append({'label':label,'note':note,'tool':tool,'input':input_data,'result':result,'error':error,'timestamp':now()}); row['updated_at']=now(); save_json('transactions.json',self.state); return row
    def commit(self,tid):
    ...
    def rollback(self,tid):
        ...
        for f in row['files']:
            ...
            if f['exists']: p.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(snap,p)
