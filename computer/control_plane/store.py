...
def ensure() -> Path:
    CP.mkdir(parents=True, exist_ok=True); return CP
def atomic_json(path: Path, data: Any) -> None:
    path.parent.mkdir(parents=True, exist_ok=True); lock_path=path.with_suffix(path.suffix+".lock"); payload=json.dumps(data,indent=2,sort_keys=True,ensure_ascii=False)+"\n"
    with lock_path.open("a+") as lock_fh:
        fcntl.flock(lock_fh.fileno(),fcntl.LOCK_EX); fd,tmp_name=tempfile.mkstemp(prefix=path.name+".tmp.",dir=str(path.parent)); tmp=Path(tmp_name)
        try:
            with os.fdopen(fd,"w",encoding="utf-8") as fh: fh.write(payload); fh.flush(); os.fsync(fh.fileno())
            os.replace(tmp,path); dir_fd=os.open(path.parent,os.O_RDONLY)
            try: os.fsync(dir_fd)
...
def save_json(name:str,data:Any)->None:
    with _STORE_LOCK: atomic_json(ensure()/name,data)
...
def redact(value:Any)->Any:
    ...
    if isinstance(value,str): return re.sub(r"(?i)(token|secret|password|cookie|authorization|api[_-]?key)\s*[:=]\s*[^\s,}]+",r"\1=<redacted>",value)
    return value
