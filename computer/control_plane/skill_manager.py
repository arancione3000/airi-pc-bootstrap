...
class SkillManager:
    ...
    def match(self,goal,limit=5):
        terms={x for x in re.findall(r'[a-z0-9_-]{3,}',str(goal).lower())}; scored=[]
        for name,row in self.state.get('skills',{}).items():
            hay=' '.join([name,row.get('description',''),' '.join(row.get('required_tools',[]))]).lower(); overlap=len(terms & set(re.findall(r'[a-z0-9_-]{3,}',hay)))
            if overlap: scored.append((overlap,name,row))
        scored.sort(key=lambda x:(x[0],x[1]),reverse=True); return {'matches':[{'name':n,'score':score,'version':r.get('version'),'status':r.get('status'),'dependencies':r.get('dependencies',[])} for score,n,r in scored[:max(1,int(limit))]]}
