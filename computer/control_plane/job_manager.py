...
class JobManager:
    def __init__(self):
        self.state = load_json(JOBS_FILE, {'version': 1, 'jobs': {}}); self.state.setdefault('version', 1); self.state.setdefault('jobs', {}); LOG_DIR.mkdir(parents=True, exist_ok=True)
    def _save(self): save_json(JOBS_FILE, self.state)
    ...
    def refresh(self):
        ...
        for row in self.state['jobs'].values():
            before = json.dumps(row, sort_keys=True); self._refresh_row(row); changed = changed or before != json.dumps(row, sort_keys=True)
        if changed: self._save()
        return self.list()
    def start(self, command: str, cwd: str = '.', timeout: int = 900, owner_task: str | None = None, scope: list[str] | None = None, allow_shell: bool = False) -> dict[str, Any]:
        if not isinstance(command, str) or not command.strip(): raise ValueError('command required')
        if not 1 <= int(timeout) <= 86400: raise ValueError('timeout must be 1..86400 seconds')
        base = safe_path(cwd)
        if not allow_shell:
            blocked = ('rm -rf','shutdown','reboot','mkfs','dd ','sudo ','curl |','wget -O-')
            if any(x in command for x in blocked): raise PermissionError('risky shell blocked; explicit allow_shell=true required')
        if scope: safe_path(cwd, missing=True)
        job_id = uuid.uuid4().hex[:12]; job_dir = LOG_DIR / job_id; job_dir.mkdir(parents=True,exist_ok=True); log_file = job_dir/'output.log'; exit_file = job_dir/'exit.code'; env = os.environ.copy(); env['AIRI_JOB_ID'] = job_id
    ...
    def list(self) -> dict[str, Any]:
        rows=[]
        for job_id in list(self.state['jobs']): rows.append(self.status(job_id))
        return {'count':len(rows),'jobs':rows}
    def attach(self, job_id: str, tail: int = 200) -> dict[str, Any]:
        row=self.status(job_id); log=Path(row['log_file']); text=log.read_text(errors='replace') if log.exists() else ''; row['attached_output']=text[-max(1,int(tail))*200:]; return row
