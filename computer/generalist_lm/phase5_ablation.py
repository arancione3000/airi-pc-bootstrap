...
def _cross_prompt_diversity(report: dict[str, Any]) -> dict[str, Any]:
    import re
    from difflib import SequenceMatcher

    traces = list(report.get("traces") or [])
    outputs = [
        re.sub(r"\\s+", " ", str(row.get("raw_output") or row.get("output") or "")).strip().lower()
        for row in traces
    ]
    counts: dict[str, int] = {}
    ...
    for left in range(len(outputs)):
        for right in range(left + 1, len(outputs)):
            pairs += 1
            sequence_similarity += SequenceMatcher(None, outputs[left], outputs[right]).ratio()
            left_tokens = set(outputs[left].split())
            right_tokens = set(outputs[right].split())
    ...
    return {
        ...
        "mean_pairwise_sequence_similarity": (
            sequence_similarity / pairs if pairs else 0.0
        ),
        "mean_pairwise_token_jaccard": (
            jaccard / pairs if pairs else 0.0
        ),
    }


def _stratified_elementary_indices(protected_rows) -> list[dict[str, Any]]:
    curriculum = _elementary_rehabilitation_rows()
    fingerprint_to_index = {
        _sft_row_fingerprint(row): index
    ...
    for stage in sorted(curriculum):
        for language in ("it", "en"):
            indices = sorted(
                fingerprint_to_index[_sft_row_fingerprint(row)]
                for row in curriculum[stage][language]
                if _sft_row_fingerprint(row) in fingerprint_to_index
            )
            if not indices:
                raise RuntimeError(
                    f"missing elementary replay stratum {stage}/{language}"
                )
            strata.append({
                "stage": stage,
                "language": language,
...
def run_ablation(state_dir: str | Path) -> dict[str, Any]:
    ...
    base_scale = max(1.0 / 128.0, min(1.0, base_scale))
    ...
    elementary_indices = list(range(len(protected_rows)))
    elementary_strata = _stratified_elementary_indices(protected_rows)
    validation_seed_indices = [0, 1, 2, 3]
    policies = [
        {
            "policy": "CURRENT_RANDOM_2",
            "replay_batch_size": 2,
        ...
        },
        {
            "policy": "STRATIFIED_8",
            "replay_batch_size": 8,
            "replay_sampling": "stratified",
            "ffn_multiplier": 1.0,
        },
        {
            "policy": "STRATIFIED_8_FFN05",
            "replay_batch_size": 8,
            "replay_sampling": "stratified",
            "ffn_multiplier": 0.5,
        },
        {
            "policy": "STRATIFIED_16",
    ...
    variants = [
        {
            ...
            "replay_sampling": policy["replay_sampling"],
    ...
    for variant in variants:
        ...
        multipliers = {
            ...
            "lm_head": float(variant.get("lm_head_multiplier", 0.50)),
        }
        ...
        for local_step in range(local_steps):
            ...
            if replay_sampling == "stratified":
                replay_indices = _sample_stratified(
                    elementary_strata,
                    batch_size=replay_batch_size,
                    seed_index=variant_retry_index,
            ...
            replay_elementary_count = len(replay_indices)
        ...
        results.append({
            ...
            "policy": str(variant.get("policy", variant["name"])),
            ...
            "anchor_headroom": {
                "before": float(
                    float(anchor["repetition_rate"]) + 0.08
                    - float(before["repetition_rate"])
                ),
                "after": float(
                    float(anchor["repetition_rate"]) + 0.08
                    - float(after["repetition_rate"])
                ),
            },
            "cross_prompt_diversity": {
                ...
                "pairwise_jaccard_delta": float(
                    after_diversity["mean_pairwise_token_jaccard"]
                    - before_diversity["mean_pairwise_token_jaccard"]
                ),
            },
            "delta": {
    ...
    return {
        "schema": 1,
        "version": "phase5-stable-recovery-v3-ablation-1",
        "read_only": True,
        ...
        "variants": [
            {
                ...
                "headroom_after": row["anchor_headroom"]["after"],
                "duplicate_rate_after": row["cross_prompt_diversity"]["after"]["exact_duplicate_rate"],
                "pairwise_jaccard_after": row["cross_prompt_diversity"]["after"]["mean_pairwise_token_jaccard"],
                "replay_rows": row["replay_rows_sampled"],
