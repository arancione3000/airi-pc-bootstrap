...
from .runtime import GeneralistRuntime, checkpoint_model_files, checkpoint_model_size
...
def research_health(state_dir: str | Path) -> dict[str, Any]:
    ...
    try:
        model_files = checkpoint_model_files(checkpoint_root)
        model_bytes = checkpoint_model_size(checkpoint_root)
        largest_model_file = max(path.stat().st_size for path in model_files)
    except Exception:
        model_files = []
        model_bytes = 0
        largest_model_file = 0
    check(
        ...
        bool(model_bytes and model_bytes <= max_persisted_bytes),
        {
            ...
            "largest_file_bytes": largest_model_file,
        },
    )
    check(
        "checkpoint:git_file_size",
        bool(model_files and largest_model_file < 90 * 1024 * 1024),
        {
            "largest_file_bytes": largest_model_file,
