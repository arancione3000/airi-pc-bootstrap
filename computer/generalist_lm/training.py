...
def train_sft_residual_recovery(
    ...
    *,
    ...
    if sampling not in {"random", "cyclic"}:
        raise ValueError("sampling_mode must be 'random' or 'cyclic'")
    ...
    for step_index in range(steps):
        optimizer.zero_grad(set_to_none=True)
        if sampling == "cyclic":
            start = (int(seed) + step_index * batch_size) % len(examples)
            indices = [
        ...
        if kl_weight > 0.0:
            ...
            if sampling == "cyclic":
                anchor_start = (
                    int(seed) * 1009 + step_index * batch_size
                ) % len(anchors)
                anchor_indices = [
